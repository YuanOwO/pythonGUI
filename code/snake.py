...
# 重置遊戲
def restart(*args):
    global score, high, speed, isDebugMode
    
    score = 0
    speed = 0.25 if isDebugMode else 1
    root.unbind('<Return>')
    text.set('Score: 0\nHigh: {}'.format(high))
    create_snake()
    create_food()
...
def create_food():
    ...
    food = (choice(list(set(range(1, size[0])) - set(range(snake[i][0], snake[-1][0] + 1)))),
            choice(list(set(range(1, size[1])) - set(range(snake[i][1], snake[-1][1] + 1)))))

# 蛇移動
def move():
    global score, high, isDebugMode, speed, size
    global snake, food
    
    speed = 0.25 if isDebugMode else 0.75 ** (score // 5)
    if snake[0]:
        snake.insert(0, None)
        snake.pop(-1)
    x, y = snake[1][0], snake[1][1]
    snake[0] = {
        'Left':  (size[0] if isDebugMode and x == 1       else x - 1, y),
        'Right': (1       if isDebugMode and x == size[0] else x + 1, y),
        'Up':    (x, size[1] if isDebugMode and y == 1       else y - 1),
        'Down':  (x, 1       if isDebugMode and y == size[1] else y + 1)
    }.get(face)
...
def turn(event):
    ...
    if reverse != event.keysym: # keysym 按鍵名稱
        face = event.keysym

# Debug
def debugMode():
    global isDebugMode, debugLabel
    isDebugMode = not isDebugMode
    debugLabel.config(text = '除錯模式' if isDebugMode else '')
...
settingMenu.add_command(label = '除錯模式', command = debugMode)
settingMenu.add_command(label = '重新開始', command = restart)
...
debugLabel = tk.Label(root, text = '', font = ('微軟正黑體', 16))
debugLabel.pack(anchor = tk.NE)
# 遊戲畫面
cv = tk.Canvas(bg = 'black', width = 400, height = 400, highlightthickness = 0)
cv.pack()

# 按鍵偵測
root.bind('<Key-g>',     debugMode)
